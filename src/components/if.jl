...
function parse_if(ps::ParseState, nested = false)
    ...
    @catcherror ps @default ps @closer ps ifelse parse_block(ps, ifblock, start_col, Tokens.Kind[Tokens.END, Tokens.ELSE, Tokens.ELSEIF])
    ...
    if ps.nt.kind == Tokens.ELSE
        ...
        push!(ret, INSTANCE(ps))
        @catcherror ps @default ps parse_block(ps, elseblock, start_col)
    end

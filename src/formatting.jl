...
function format(ps::ParseState)
    if ps.formatcheck
        if isoperator(ps.t)
            ...
            if prec == 8 || prec == 13 || prec == 14 || prec == 15
                ...
                if ps.ws.kind != EmptyWS
                    push!(ps.hints, FormatHint{DeleteWhiteSpace}(ps.ws.startbyte+1:ps.ws.endbyte+1))
                end
            else
                if ps.lws.kind == EmptyWS
                    push!(ps.hints, FormatHint{AddWhiteSpace}(ps.t.startbyte))
                end
                if ps.ws.kind == EmptyWS
                    push!(ps.hints, FormatHint{AddWhiteSpace}(ps.t.endbyte+1))
                end
            end
        ...
        elseif ps.t.kind == Tokens.ELSE && ps.ws.kind == WS && ps.nt.kind == Tokens.IF
            push!(ps.hints, FormatHint{Useelseif}(ps.t.startbyte))
            Useelseif
        end
    end
end